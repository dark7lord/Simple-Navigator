...
const std::string Graph::ToString(const std::string &name) const {
  if (size_matrix_ == 0) throw EmptyGraphException();

  std::ostringstream result;
  std::string padding = "  ";

  result << (is_digraph ? "digraph " : "graph ") << name << " {" << std::endl;
...
void Graph::ExportGraphToDot(const std::string &filename) {
  ...
  output_file << this->ToString(filename);
  output_file.close();
}
...
void Graph::PrintGraphInBrowser() {
  ...
  std::string encoded_graph = UrlEncode(this -> ToString("test"));
